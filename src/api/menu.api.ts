...
export const getMenusByRestaurant = async (restaurantId: number): Promise<MenuResponse[]> => {
  const res = await instance.get<MenuResponse[]>(`${API_URL}/restaurants/${restaurantId}/menus`);
  return res.data;
};

export const getMenu = async (id: number): Promise<MenuResponse> => {
  const res = await instance.get<MenuResponse>(`${API_URL}/menus/${id}`);
  return res.data;
};

export const createMenu = async (data: MenuData): Promise<MenuResponse> => {
  const res = await instance.post<MenuResponse>(`${API_URL}/menus`, data);
  return res.data;
};

export const updateMenu = async (id: number, data: MenuData): Promise<MenuResponse> => {
  const res = await instance.put<MenuResponse>(`${API_URL}/menus/${id}`, data);
  return res.data;
};

export const deleteMenu = async (id: number): Promise<void> => {
  await instance.delete(`${API_URL}/menus/${id}`);
};