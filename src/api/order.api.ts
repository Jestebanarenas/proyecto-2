...
export const createOrder = async (data: OrderData): Promise<OrderResponse> => {
  ...
  const res = await instance.post<OrderResponse>(`${API_URL}/orders`, payload);
  return res.data;
};

export const getOrders = async (): Promise<OrderResponse[]> => {
  const res = await instance.get<OrderResponse[]>(`${API_URL}/orders`);
  return res.data;
};

export const deleteOrder = async (orderId: number): Promise<void> => {
  await instance.delete(`${API_URL}/orders/${orderId}`);
};

export const updateOrder = async (orderId: number, data: Partial<OrderData>): Promise<OrderResponse> => {
  const res = await instance.put<OrderResponse>(`${API_URL}/orders/${orderId}`, data);
  return res.data;
};